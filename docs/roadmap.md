...
Tracks planned work beyond v1.0.x. Grouped by "in flight" (v1.3), "backlog" (not yet scheduled), and "not planned" (explicitly rejected, included so users don't have to ask).
...
Items the project wants, in rough priority order, but that are not scheduled into v1.3.
...
- **Carbon-intensity overlay.** "Deploying in `eu-west-1` vs `us-east-1` saves N kg CO2/yr at cost difference of $M." Needs a new external data dependency (Electricity Maps / WattTime). Revisit for v1.3 when regulatory pressure (EU AI Act enforcement) forces the requirement.
